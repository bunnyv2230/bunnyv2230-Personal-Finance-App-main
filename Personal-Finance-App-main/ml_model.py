...
def fetch_expense_data(user_id):
    ...
    db_path = os.path.join(basedir, 'personal_finance.db')
...
def cluster_expenses(user_id, method='kmeans', n_clusters=3):
    ...
    # Replace null descriptions with empty strings instead of filtering them out
    ...
    try:
        ...
        lda_matrix = np.zeros((len(data), n_clusters))

    # Sentiment Analysis
    data['sentiment'] = data['description'].apply(lambda x: TextBlob(x).sentiment.polarity if x else 0.0)

    # Combine features
    combined_features = np.hstack((data[['amount']].values, lda_matrix, data[['sentiment']].values.reshape(-1, 1)))

    # Dynamically cap n_clusters if there are fewer samples than requested clusters
    n_samples = len(data)
    ...
    # Clustering
    if method == 'kmeans':
        model = KMeans(n_clusters=actual_clusters, random_state=42)
    elif method == 'hierarchical':
        model = AgglomerativeClustering(n_clusters=actual_clusters)
    elif method == 'dbscan':
        model = DBSCAN(eps=0.5, min_samples=min(2, n_samples))
    else:
